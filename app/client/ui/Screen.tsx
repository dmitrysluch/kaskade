 ...
 *
 * Пустой полноэкранный слой: ни линейки, ни статуса, ни истории терминала,
 * ни списка команд, ни рамки. Реплики при этом сохраняют обычные цвета
 * говорящих — это по-прежнему сцена, просто игроку в ней не дают руля.
 *
 * Рамки нет намеренно: рамка значит «предъявленная запись», а монтаж
 * показывает событие. Это и отличает его от титра, при том что ведут себя
 * они одинаково — ждут нового `Enter`.
 */
export function MontageScreen({
  ...
  cols: number;
  rows: number;
  /** Готовые строки потока: цвета говорящих уже расставлены. */
  lines: Seg[][];
  hint?: string;
}) {
  /*
   * Подсказка стоит в правом нижнем поле, как у титра, и место под неё
   * зарезервировано: иначе длинный кадр наезжает на неё текстом, а обрезанный
   * низ выглядит как строка, которую забыли дописать.
   *
   * Блок стоит по центру оставшегося; если он выше экрана, начинается сверху —
   * потерять низ лучше, чем начало.
   */
  const room = Math.max(1, rows - BOTTOM_ROWS - 1);
  const shown = lines.slice(0, room);
  const above = Math.max(0, Math.floor((room - shown.length) / 2));
  ...
  return (
    <>
      {Array.from({ length: rows }, (_, i) => (
        <Line key={i} segs={(i === rows - 2 ? foot : shown[i - above]) ?? []} cols={cols} />
      ))}
    </>
  );
}
